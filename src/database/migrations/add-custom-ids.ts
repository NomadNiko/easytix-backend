...
class MigrationService {
  ...
  async migrateCategories() {
    ...
    for (const category of categories) {
      ...
      console.log(`Updated category "${category.name}" with customId: ${customId}`);
      sequence++;
    }
  ...
  async updateSystemDefaults() {
    ...
    try {
      systemDefaultModel = this.queueModel.db.model(SystemDefaultSchemaClass.name);
    } catch (error) {
    ...
    if (queueDefault) {
      ...
      if (queue && queue.customId) {
        ...
        console.log(`Updated DEFAULT_QUEUE_ID to use customId: ${queue.customId}`);
      }
    }
    ...
    if (categoryDefault) {
      ...
      if (category && category.customId) {
        ...
        console.log(`Updated DEFAULT_CATEGORY_ID to use customId: ${category.customId}`);
      }
    }
