...
export class SystemDefaultsService {
  constructor(
    private readonly systemDefaultRepository: SystemDefaultRepository,
  ) {}

  async create(
    createSystemDefaultDto: CreateSystemDefaultDto,
  ): Promise<SystemDefault> {
    // Check if key already exists
    const existing = await this.systemDefaultRepository.findByKey(
      createSystemDefaultDto.key,
    );
    if (existing) {
      throw new ConflictException(
        `System default with key "${createSystemDefaultDto.key}" already exists`,
      );
    }
  ...
  async update(
    id: string,
    updateSystemDefaultDto: UpdateSystemDefaultDto,
  ): Promise<SystemDefault> {
    ...
    const updatedSystemDefault = await this.systemDefaultRepository.update(
      id,
      updateSystemDefaultDto,
    );
  ...
  async getDefaultQueueId(): Promise<string | null> {
    const defaultQueue = await this.findByKey(
      SystemDefaultKey.DEFAULT_QUEUE_ID,
    );
    return defaultQueue?.value || null;
  }

  async getDefaultCategoryId(): Promise<string | null> {
    const defaultCategory = await this.findByKey(
      SystemDefaultKey.DEFAULT_CATEGORY_ID,
    );
